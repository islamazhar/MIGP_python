...
for n in N_HYBRID_VALUES:
    ...
    for i in range(2):
        ...
        for line in file_r:
            ...
            tweaks_1 = get_tweaks_rules_edr(words[0],n)
            tweaks_2 = get_tweaks_rules_edr(words[1],n)
...
with open('../models/count_edr_hybrid.json') as f:
    count_edr_hybrid = [tuple(x) for x in json.load(f)]
    
c_var = []
s_var = []
max_len = 40
for elem in count_edr_hybrid:
...
def get_tweaks_rules_hybrid(word,n,htype):
    ...
    for elem in elems:
        if len(tweaks) == MAX_TWEAKS:
            break
        tw = apply_tweak_edr(word,rules_list[elem])   
        if tw == word:
            continue
        else:
            tweaks.add(tw)
